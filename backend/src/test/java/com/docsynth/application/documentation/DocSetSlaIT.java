...
/**
 * Integration test for SC-002: a 50-endpoint DocSet is browsable within
 * 60 seconds of spec ingestion.
 */
@SpringBootTest
@ActiveProfiles("test")
class DocSetSlaIT {
    ...
    @Test
    void fifty_endpoints_browsable_within_60s() throws Exception {
        when(codeClient.generate(any(), any(), any(), anyString())).thenReturn(
            DocGeneratorProto.GenerateCodeExampleResponse.newBuilder()
                .setCode("curl ... ")
                .setLanguage("curl")
                .setPromptTemplateVersion("v1.0")
                .setConfidence(0.9f)
                .build()
        );
        when(descClient.enhance(any(), anyString())).thenReturn(
            DocGeneratorProto.EnhanceDescriptionResponse.newBuilder()
                .setEnhanced("Enhanced description.")
                .setPromptTemplateVersion("v1.0")
                .setConfidence(0.8f)
                .build()
        );
        when(ssg.build(any(), any())).thenReturn(50);
        when(manifestEmitter.emit(any(), anyString())).thenReturn("https://blob/manifest.json");
